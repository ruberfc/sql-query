                ...
                IF @TotalNotasCurso = @TotalCursosMatriculados  AND  @TotalNotasCurso > 0 AND @TotalCursosMatriculados > 0 BEGIN
                    ...
                    BEGIN
                        ...
                        IF EXISTS(SELECT * from SGA.dbo.PensionesxCobrar where SeriDeud = TRIM(@DeudaSeri) and NumDeud = TRIM(@DeudaNum) and Comprobante LIKE 'B%' or Comprobante LIKE 'F%') BEGIN
                            ...
                            select top 10 * from SGA.dbo.TipOper
                            ...
                            BEGIN
                                ...
                                VALUES
                                (
                                    @usuario_Serie, @NumOperChar, @DetItem, '6595257', 
                                    -- 'H', 
                                    -- @DetImporte
                                    case 
                                        when @DescuentoDoble = '1' THEN 'H'
                                        else 'H'
                                    end,
                                    case 
                                        when @DescuentoDoble = '1' AND @DetImporte < 0 THEN -@DetImporte
                                        else @DetImporte
                                    end 
                                    , @DetNumCuota, @MatriculaAnioMax,
                                ...
                                select * from SGA.dbo.Descuento_doble where Seriedeud = '888802449789'
                                select top 10 * from SGA.dbo.DetOper where DocRef = '888802449789'
